...
class TorchMXL(nn.Module):
    ...
    def elbo(self, alt_attr, context_attr, obs_choices, alt_avail, obs_mask, alt_ids, indices):
        ...
        q_alpha, q_zeta, q_L_Omega_diag, q_L_Omega_offdiag, q_beta, beta_offsets = self.forward(alt_attr, context_attr, obs_choices, alt_avail, alt_ids)
        ...
        if self.dcm_spec.model_type == 'MNL':
            ...
            beta_resp = torch.cat([params_resp[:,self.dcm_spec.param_id_map_by_alt[i]] for i in range(self.num_alternatives)], dim=-1)
            ...
            params_resp = params_resp + beta_offsets.transpose(0,1)
            beta_resp = torch.cat([params_resp[:,:,self.dcm_spec.param_id_map_by_alt[i]] for i in range(self.num_alternatives)], dim=-1)
        else:
